...
<head>
    <meta charset="UTF-8" />
    <title>Tambah Rekam Medis KB - {{ $pasien->nama_pasien }}</title>
    <style>
        body {
...
<body>
    ...
    <h1>Tambah Rekam Medis KB - {{ $pasien->nama_pasien }}</h1>

    <form action="{{ route('pasien.rekam-medis-kb.store', $pasien->id) }}" method="POST">
    @csrf

        <div class="form-group">
            <label>Tanggal Datang:</label>
            <input type="date" name="tanggal_datang" value="{{ old('tanggal_datang') }}" required>
        </div>
        ...
        <div class="form-group">
            ...
            <textarea name="pemeriksaan_dan_tindakan">{{ old('pemeriksaan_dan_tindakan') }}</textarea>
        </div>
